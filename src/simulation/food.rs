...
pub struct FoodPatch {
    ...
    heading: f32,
    /// User-dropped food: *placed* (doesn't drift), but otherwise a normal
    /// renewable patch — it regrows when uneaten and is grazed down when eaten,
    /// like the rest of the self-sustaining field.
    placed: bool,
}
...
impl FoodField {
    ...
    pub fn update(&mut self, seed: u64, step: u32) {
        ...
        for (i, p) in self.patches.iter_mut().enumerate() {
            // Ambient patches wander; user-placed food stays where it was dropped.
            if !p.placed {
            ...
            // Every patch regrows toward capacity in place — the self-sustaining
            // field: an uneaten patch grows back, a grazed one recovers once the
            // crowd disperses. Regrowth is a fraction of capacity, so a richer
            // patch refills proportionally faster.
    ...
    /// Drop a renewable food patch at `(x, y)` — the user feeding the world. It's
    /// *placed* (doesn't drift) but otherwise part of the self-sustaining field:
    /// it grows back when uneaten and is grazed down (fast, when crowded) when
    /// eaten. Smaller than an ambient patch ("not a big block") but rich, so it's
    /// worth swarming. Capped so drops can't pile up forever (oldest drops out).
    pub fn drop_food(&mut self, x: f32, y: f32) {
        const MAX_PLACED: usize = 16;
        if self.patches.iter().filter(|p| p.placed).count() >= MAX_PLACED {
            if let Some(i) = self.patches.iter().position(|p| p.placed) {
                self.patches.remove(i);
            }
        }
        let cap = self.cfg.patch_peak * 1.5;
        let b = self.half_world;
        self.patches.push(FoodPatch {
            x: x.clamp(-b, b),
            y: y.clamp(-b, b),
            radius: self.cfg.patch_radius * 0.6,
            intensity: cap,
            capacity: cap,
            heading: 0.0,
            placed: true,
