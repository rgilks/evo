...
Each `get_stats()` reports the population count, a per-colour classification, average genetic metrics (speed, size, efficiency, reproduction rate, sense radius), population density, and world-centre drift, serialized to JS. The current frontend doesn't surface these (see [BACKLOG.md](../BACKLOG.md) — "Trim unused stats").
